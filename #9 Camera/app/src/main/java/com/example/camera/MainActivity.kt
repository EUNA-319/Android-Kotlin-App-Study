...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var curPhotoPath: String // 문자열 형태의 사진 경로 값 (lateinit: 초기 값을 null로 시작하고 싶을 때)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val camera_btn = findViewById<Button>(R.id.btn_camera)
    ...
    private fun setPermission() {
        val permission = object: PermissionListener {
            override fun onPermissionGranted() { // 설정해 놓은 위험 권한들이 허용되었을 경우 이 곳을 수행함
                Toast.makeText(this@MainActivity, "권한이 허용 되었습니다", Toast.LENGTH_SHORT).show()
            }

            override fun onPermissionDenied(deniedPermissions: MutableList<String>?) { // 설정해 놓은 위험 권한들 중 거부를 한 경우 이곳을 수행
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == REQUEST_IMAGE_CAPTURE && resultCode == Activity.RESULT_OK){ // 이미지를 성공적으로 가져왔다면
            ...
            val profile_iv = findViewById<ImageView>(R.id.iv_profile)
